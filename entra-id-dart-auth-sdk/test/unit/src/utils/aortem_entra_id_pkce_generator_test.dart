...
import 'package:entra_id_dart_auth_sdk/src/utils/aortem_entra_id_pkce_generator.dart';
...
void main() {
  group('AortemEntraIdPkceGenerator', () {
    test('should generate a valid code verifier and challenge pair', () {
      ...
      expect(pkcePair['code_verifier']!.length, 43);  // PKCE spec requires 43 characters for code verifier
    });
    ...
    test('should generate different code verifiers on each call', () {
      ...
      expect(pkcePair1['code_verifier'], isNot(equals(pkcePair2['code_verifier'])));
    });
    ...
    test('should generate a URL-safe base64 encoded code challenge', () {
      ...
      expect(codeChallenge, matches(r'^[A-Za-z0-9\-_]+$'));  // Only URL-safe base64 characters
      expect(codeChallenge.length, lessThan(128));  // PKCE challenge is typically short
    });
  });
}
